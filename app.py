...
def process_command(command):
    ...
    if "play" in command:
        ...
        response_data["text"] = f"Sure, playing {song} for you."
       ...
       youtube_url = f"https://www.google.com"
       response_data["text"] = f"opening Chrome 🚀"
       response_data["action_url"] = youtube_url
    elif "open grocery store" in command:
        grocery_url = "https://elite-mart.lovable.app/"
        response_data["text"] = "Opening grocery store 🛒"
        response_data["action_url"] = grocery_url
    elif "date" in command:
        response_data["text"] = "Today's date is: " + datetime.datetime.now().strftime('%Y-%m-%d').strftime('%A')
        
    elif "what is" in command or "day is" in command:
        try:
            target_date = None
